...
class Base64Codec(val char62: Char = '+', val char63: Char = '/', val padChar: Char = '=',
  ...
  /** Decoder. Supports all the variants produced by the encoder above, but
    ...
    * be garbage. */
  def decode(data: String)(implicit eh: ExceptionHandler): eh.![Array[Byte], Exception] =
    eh.wrap {
  ...
  def unapply(in: String): Option[Array[Byte]] = Some(decode(in)(strategy.throwExceptions))
}
...
object Hex {
  ...
  def decode(s: String)(implicit eh: ExceptionHandler): eh.![Array[Byte], Exception] = eh.wrap {
