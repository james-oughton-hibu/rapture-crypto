...
class Base64Codec(val char62: Char = '+', val char63: Char = '/', val padChar: Char = '=',
  ...
  /** Decoder. Supports all the variants produced by the encoder above, but
    ...
    * be garbage. */
  def decode(data: String)(implicit mode: Mode[CryptoMethods]): mode.Wrap[Array[Byte], Exception] =
    mode.wrap {
...
object Hex {
  ...
  def decode(s: String)(implicit mode: Mode[CryptoMethods]): mode.Wrap[Array[Byte], Exception] = mode.wrap {
